...
LastEditTime: 2024-03-08 08:30:54
FilePath: /prostate158-main/prostate158/network/davitUNETR_ECSC.py
...
class davitUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.scconv_depth = scconv_depth
        for i_num, i in enumerate([24, 48, 96, 192]):
            setattr(self, f"ScConv_list_{i}", nn.ModuleList())
            for j in range(self.scconv_depth[i_num]):
                getattr(self, f"ScConv_list_{i}").append(ScConv(i))
    ...
    def forward(self, x_in):
        ...
        # skip connection
        for blk in self.ScConv_list_24:
            enc1 = blk(enc1) 
        for blk in self.ScConv_list_48:
            enc2 = blk(enc2)
        for blk in self.ScConv_list_96:
            enc3 = blk(enc3)
        for blk in self.ScConv_list_192:
            enc4 = blk(enc4)

...
# from networks.biformer.biformerattn import BiLevelRoutingAttention

rearrange, _ = optional_import("einops", name="rearrange")


#--------------------------Biformer attention block----------
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x, window_size=7, ret_attn_mask=False):
        """
        x: NHWC tensor
        ...
        N, H, W, D, C = x.size()
        assert H%window_size== 0 and W%window_size== 0 and D%window_size== 0#


        # patchify, (n, p^2, w, w, c), keep 2d window as we need 2d pooling to reduce kv size
        x = rearrange(x, "n (j h) (i w) (k d) c -> n (j i k) h w d c", j=window_size, i=window_size, k=window_size) # torch.Size([2, 56, 56, 64]) -> torch.Size([2, 49, 8, 8, 64])

        #################qkv projection###################
        # q: (n, p^2, w, w, c_qk) torch.Size([2, 343, 7, 7, 7, 24])
        # kv: (n, p^2, w, w, c_qk+c_v) torch.Size([2, 343, 7, 7, 7, 48])
        # NOTE: separte kv if there were memory leak issue caused by gather
        q, kv = self.qkv(x) # torch.Size([2, 49, 8, 8, 64]), # torch.Size([2, 49, 8, 8, 128])

        # pixel-wise qkv
        # q_pix: (n, p^2, w^2, c_qk)
        # kv_pix: (n, p^2, h_kv*w_kv, c_qk+c_v)
        q_pix = rearrange(q, 'n p3 h w d c -> n p3 (h w d) c')
        kv_pix = self.kv_down(rearrange(kv, 'n p3 h w d c -> (n p3) c h w d'))
        kv_pix = rearrange(kv_pix, '(n j i k) c h w d -> n (j i k) (h w d) c', j=window_size, i=window_size, k=window_size)

        q_win, k_win = q.mean([2, 3, 4]), kv[..., 0:self.qk_dim].mean([2, 3, 4]) # window-wise qk, (n, p^2, c_qk), (n, p^2, c_qk)

        ##################side_dwconv(lepe)##################
        # NOTE: call contiguous to avoid gradient warning when using ddp
        lepe = self.lepe(rearrange(kv[..., self.qk_dim:], 'n (j i k) h w d c -> n c (j h) (i w) (k d)', j=window_size, i=window_size, k=window_size).contiguous())
        lepe = rearrange(lepe, 'n c (j h) (i w) (k d) -> n (j h) (i w) (k d) c', j=window_size, i=window_size, k=window_size)
        ...
        out = rearrange(out, '(n j i k) m (h w d) c -> n (j h) (i w) (k d) (m c)', j=window_size, i=window_size, k=window_size,
                        h=H//window_size, w=W//window_size, d=D//window_size)
...
class AttentionLePE(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        _, H, W, D, _ = x.size()
        x = rearrange(x, 'n h w d c -> n (h w d) c')
        ...
        lepe = self.lepe(rearrange(x, 'n (h w d) c -> n c h w d', h=H, w=W, d=D))
        lepe = rearrange(lepe, 'n c h w d -> n (h w d) c')
        ...
        x = rearrange(x, 'n (h w d) c -> n h w d c', h=H, w=W, d=D)
        return x
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if topk > 0:
            self.attn = BiLevelRoutingAttention(
                ...
                num_heads=num_heads,
                n_win=7,
                qk_dim=qk_dim, # [64, 128, 256, 512]
                qk_scale=None, 
                kv_per_win=kv_per_win, # [2, 2, -1, -1]
                kv_downsample_ratio=kv_downsample_ratio, # [4, 2, 1, 1]
                kv_downsample_kernel=kv_downsample_kernel,  # [4, 2, 1, 1]
                kv_downsample_mode='identity',
                topk=topk,  # [1, 4, 16, -2] 
            ...
            self.attn = AttentionLePE(dim, side_dwconv=5)
    ...
    def forward_part1(self, x, mask_matrix):
        x_shape = x.size() # torch.Size([2, 48, 48, 48, 48])
        x = self.norm1(x)
        if len(x_shape) == 5:
        ...
        x_windows = shifted_x # attn内部进行patchify
        if self.topk >0:
            attn_windows = self.attn(x_windows, window_size=window_size[0], ret_attn_mask=False)
        else:
            attn_windows = self.attn(x_windows)
        ## attn内部进行patch reverse
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        qk_dim:int,
        kv_per_win:int,
        kv_downsample_ratio:int,
        kv_downsample_kernel: int,
        topk: int,
        mlp_ratio: float = 4.0,
        qkv_bias: bool = False,
        drop: float = 0.0,
        ...
        #-----------------biformer--------
        # self.qk_dims =  [64, 128, 256, 512]
        # self.kv_per_wins = [2, 2, -1, -1]
        # self.kv_downsample_ratios = [4, 2, 1, 1]
        # self.kv_downsample_kernels = [4, 2, 1, 1]
        # self.topks = [1, 4, 16, -2]  # [1, 4, 16, -2] 
    ...
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                qk_dim = self.qk_dims[i_layer],
                kv_per_win=self.kv_per_wins[i_layer],
                kv_downsample_ratio=self.kv_downsample_ratios[i_layer],
                kv_downsample_kernel=self.kv_downsample_kernels[i_layer],
                topk=self.topks[i_layer],
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                drop=drop_rate,

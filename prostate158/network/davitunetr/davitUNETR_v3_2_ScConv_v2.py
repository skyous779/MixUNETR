...
LastEditTime: 2024-02-26 10:10:07
FilePath: /prostate158-main/prostate158/network/davitUNETR_v3_2_ScConv_v2.py
...
from prostate158.block.ScConv3d import ScConv
...
class davitUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.down_1 =  Down(
            spatial_dims, 
            in_chns=feature_size, 
            out_chns=feature_size, 
        ...
        self.down_3 =  Down(
            spatial_dims, 
            in_chns=2*feature_size, 
            out_chns=4*feature_size, 
            act=("LeakyReLU", {"negative_slope": 0.1, "inplace": True}), 
            norm=("instance", {"affine": True}),
            bias=True, 
            dropout=0.0)
        ...
        self.down_4 =  Down(
            spatial_dims, 
            in_chns=4*feature_size, 
            out_chns=8*feature_size, 
            act=("LeakyReLU", {"negative_slope": 0.1, "inplace": True}), 
            norm=("instance", {"affine": True}),
            bias=True, 
            dropout=0.0)
        ...
        self.down_5 =  Down(
            spatial_dims, 
            in_chns=8*feature_size, 
            out_chns=16*feature_size, 
            act=("LeakyReLU", {"negative_slope": 0.1, "inplace": True}), 
            norm=("instance", {"affine": True}),
            bias=True, 
            dropout=0.0)
        ...
        for i in [24, 48, 96, 192]:
            setattr(self, f"ScConv{i}", ScConv(i))

        for i in [24, 48]:
            setattr(self, f"ScConv2_{i}", ScConv(i))

        for i in [24]:
            setattr(self, f"ScConv3_{i}", ScConv(i))
    ...
    def forward(self, x_in):
        ...
        enc1_ = self.ScConv3_24(self.ScConv2_24(self.ScConv24(enc1))) + enc1

        enc2 = self.layers2[0](self.encoder3(self.down_2(enc1)))
        enc2 = self.proj_out(enc2, normalize=True)
        enc2_ = self.ScConv2_48(self.ScConv48(enc2)) + enc2

        enc3 = self.layers3[0](self.encoder4(self.down_3(enc2))) #12
        enc3 = self.proj_out(enc3, normalize=True)
        enc3_ = self.ScConv96(enc3) + enc3

        enc4 = self.layers4[0](self.encoder5(self.down_4(enc3)))
        enc4 = self.proj_out(enc4, normalize=True)
        enc4_ = self.ScConv192(enc4) + enc4

        # chvit_out = self.ChViT([enc1, enc2, enc3, enc4])

        # dec4 = self.encoder10(hidden_states_out[4])
        dec3 = self.decoder5(self.down_5(enc4), enc4_)
        dec2 = self.decoder4(dec3, enc3_)
        dec1 = self.decoder3(dec2, enc2_)
        dec0 = self.decoder2(dec1, enc1_)

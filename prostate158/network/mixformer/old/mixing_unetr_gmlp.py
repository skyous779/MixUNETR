...
class SpatialGatingUnit(nn.Module):
    def __init__(self, d_ffn, seq_len):
        ...
        self.spatial_proj = nn.Conv3d(seq_len, seq_len, kernel_size=1)
        nn.init.constant_(self.spatial_proj.bias, 1.0)

    def forward(self, x):
        u, v = x.chunk(2, dim=-1)
        v = self.norm(v)
        v = self.spatial_proj(v.permute(0,4,1,2,3))
        out = u * v.permute(0,2,3,4,1)
        return out

class SpatialGatingUnit_3d(nn.Module):
    def __init__(self, d_ffn, seq_len):
        super().__init__()
        self.norm = nn.LayerNorm(d_ffn)
    ...
    def forward(self, x):
        u, v = x.chunk(2, dim=-1)
        v = self.norm(v)
        v = self.spatial_proj(v.permute(0,2,1))
...
class gMLPBlock(nn.Module):
    def __init__(self, d_model, d_ffn, seq_len):
        ...
        self.channel_proj1 = nn.Linear(d_model, d_ffn * 2) # 加两倍d_ffn给sgu
        self.channel_proj2 = nn.Linear(d_ffn, d_model)
        self.sgu = SpatialGatingUnit_3d(d_ffn, seq_len)
...
class MixingUNETR_V2(nn.Module):
    """
...
class MixAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.gmlp = gMLP(dim, dim*2, dim*2, num_layers=6)
        self.proj_drop = nn.Dropout(proj_drop)
        trunc_normal_(self.relative_position_bias_table, std=0.02)
        self.softmax = nn.Softmax(dim=-1)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        # self.gmlp = gMLP(d_model=dim, d_ffn=mlp_hidden_dim, seq_len=mlp_hidden_dim, num_layers=1)

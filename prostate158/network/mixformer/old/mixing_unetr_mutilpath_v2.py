...
class MultiMixingUNETR_v2(nn.Module):
    """
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        downsample_v2: Optional[nn.Module] = None,
        use_checkpoint: bool = False,
        num_paths: int = 2,
    ) -> None:
        """
        ...
        self.DWConv3d_BN_list = nn.ModuleList()
        for num_path in range(self.num_paths):
            self.DWConv3d_BN_list.append(DWConv3d_BN(dim, 
                    dim,
                    kernel_size=3,
                    stride=1
                    ))
        ...
        # self.DWConv3d_BN = DWConv3d_BN(dim, 
        #             dim,
        #             kernel_size=3,
        #             stride=1
        #             )
        
        self.aggregate = Conv3d_BN(dim*2*self.num_paths,
                          dim*2,
                          act_layer=nn.Hardswish)
        
        self.block_paths = nn.ModuleList()
        for path in  range(self.num_paths):
            ...
            self.block_paths.append(blocks)
        ...
        #             for i in range(depth)
        #         ])
        # self.block_paths = [self.blocks1, self.blocks2]


        self.downsample_x = downsample_v2
        self.downsample = downsample
        if callable(self.downsample):
            self.downsample = downsample(dim=dim, norm_layer=norm_layer, spatial_dims=len(self.window_size))

        if callable(self.downsample_x):
            self.downsample_x = downsample_v2(dim=dim, norm_layer=norm_layer, spatial_dims=len(self.window_size))

    def forward_single_path(self, x, path_index):
        x_shape = x.size()
        if len(x_shape) == 5:
            b, c, d, h, w = x_shape
            window_size, shift_size = get_window_size((d, h, w), self.window_size, self.shift_size)
            ...
            for blk in self.block_paths[path_index]:
                x = blk(x, attn_mask)
            x = x.view(b, d, h, w, -1)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_paths):
            x_inputs.append(self.DWConv3d_BN_list[2*i+1](
                self.DWConv3d_BN_list[2*i](x_inputs[-1])
                ))


        for i in range(self.num_paths):
            x_outputs.append(self.forward_single_path(x_inputs[i+1], i))  # x_inputs第0个为x,所以输入需要加一
        b, c, d, h, w = x.size()
        x = rearrange(x, "b c d h w -> b d h w c")
        x = self.downsample_x(x)
        x = rearrange(x, "b d h w c -> b c d h w")

        out_concat = torch.cat(x_outputs, dim=1)

        x  = x + self.aggregate(out_concat)
        return x
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        down_sample_mod_x = look_up_option("mergingv2", MERGING_MODE) if isinstance("mergingv2", str) else downsample
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                use_checkpoint=use_checkpoint,
                downsample_v2 = down_sample_mod_x,
            )
...
class Conv3d_BN(nn.Module):
    ...
    def __init__(
        ...
        self.conv = torch.nn.Conv3d(in_ch,
                                    out_ch,
                                    kernel_size,
                                    stride,
                                    pad,
...
class ResBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = Conv3d_BN(in_features,
                               hidden_features,
                               act_layer=act_layer)
        self.dwconv = nn.Conv3d(
            hidden_features,
        ...
        self.act = act_layer()
        self.conv2 = Conv3d_BN(hidden_features, out_features)

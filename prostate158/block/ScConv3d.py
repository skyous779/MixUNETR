...
Date: 2024-02-24 09:10:18
LastEditors: skyous 1019364238@qq.com
LastEditTime: 2024-02-24 09:49:53
FilePath: /prostate158-main/tools/ScConv3d.py
Description: 
...
class SRU(nn.Module):
    def __init__(self,
        ...
        self.gn             = nn.GroupNorm( num_channels = oup_channels, num_groups = group_num ) \
                            if torch_gn else GroupBatchnorm3d(c_num = oup_channels, group_num = group_num)
...
class CRU(nn.Module):
    ...
    def __init__(self, 
        ...
        #up
        self.GWC            = nn.Conv3d(up_channel//squeeze_radio, 
                                        op_channel,kernel_size=group_kernel_size, 
                                        stride=1,padding=group_kernel_size//2, 
                                        groups = group_size if (up_channel//squeeze_radio) % 2 == 0 else 1) 
                            
        self.PWC1           = nn.Conv3d(up_channel//squeeze_radio, op_channel,kernel_size=1, bias=False)
        #low
        self.PWC2           = nn.Conv3d(low_channel//squeeze_radio, op_channel-low_channel//squeeze_radio,kernel_size=1, bias=False)

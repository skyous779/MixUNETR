...
from .dou_loss import BoundaryDoULoss3D, BoundaryDoUCELoss3D, BoundaryDoULoss3D_V2
...
train_epoch_loss = 0
old_epoch = 1
iteration_per_epoch = 0

def loss_logger(engine): 
    global train_epoch_loss
    global old_epoch
    global iteration_per_epoch
    ...
    if epoch > old_epoch:
        train_epoch_loss = train_epoch_loss/(iteration_per_epoch + 1)
...
def metric_logger(engine): 
    ...
    if engine.state.epoch > 1: # only key metric is calcualted in 1st epoch, needs fix
        ...
        wandb.log(engine.state.metrics, step=engine.state.epoch)
...
def get_train_handlers(
    ...
    train_handlers=[
        ValidationHandler(
            validator=evaluator, 
            interval=1,  #  interval: do validation every N epochs or every N iterations during training.
            epoch_level=True 
        ),
        StatsHandler(
            tag_name="train_loss", 
        ...
        StatsHandler(
            ...
            epoch_print_logger=loss_logger,
        ), 
        TensorBoardStatsHandler(
            log_dir=config.log_dir,
            tag_name="train_loss",
...
class SegmentationTrainer(monai.engines.SupervisedTrainer): 
    ...
    def __init__(self, 
        ...
        if config.loss_type == 'bdouloss': 
            loss_fn=BoundaryDoULoss3D(
                n_classes=config.model.out_channels
                ).to(device=config.device)
            
        elif config.loss_type == 'bdouCEloss': 
            loss_fn=BoundaryDoUCELoss3D(
                n_classes=config.model.out_channels
                ).to(device=config.device)

        elif config.loss_type == 'bdoulossv2': 
            loss_fn=BoundaryDoULoss3D_V2(
                n_classes=config.model.out_channels
                ).to(device=config.device)
    ...
    def _prepare_dirs(self)->None:
        ...
        # there is the cause for why genearting file in the ./ !!!
        # os.makedirs(self.config.run_id, exist_ok=True)
        os.makedirs(os.path.join(self.config.save_path, self.config.run_id), exist_ok=True)
        with open(
            os.path.join(
    ...
    def run(self, try_resume_from_checkpoint=False) -> None:
        ...
        if self.config.resume:
            try: 
                ...
                print(f"resuming from previous checkpoint at {self.config.checkpoint_path}")

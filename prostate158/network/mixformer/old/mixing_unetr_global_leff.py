...
class LeFF(nn.Module):
    """
    Modified Locally enhanced feed forward layer from "Yuan et al
    ...
    def __init__(self, dim, reso, scale=4, depth_kernel=3):
        super().__init__()
        ...
        self.up_proj = nn.Sequential(
            ...
            nn.GELU(),
            Rearrange('b c (d h w) -> b c d h w', d=reso, h=reso, w=reso),
        )
        ...
        self.depth_conv = nn.Sequential(
            ...
            nn.BatchNorm3d(scale_dim),
            nn.GELU(),
            Rearrange('b c d h w -> b (d h w) c', d=reso, h=reso, w=reso),
        )
        ...
        self.down_proj = nn.Sequential(
            ...
            # Rearrange('(b D c) -> b (D1 D2 D3) c', D1=n, D2=n, D3=n),
        )
...
class GCMixingUNETR_v2(nn.Module):
    """
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = LeFF(dim=dim, reso=reso)
    ...
    def forward_part2(self, x):
        x = self.norm2(x)
        x = Rearrange('b i j k c -> b (i j k) c')(x)
        x = self.mlp(x)
        x = Rearrange('b (D1 D2 D3) c -> b D1 D2 D3 c',  D1=self.reso, D2=self.reso, D3=self.reso)(x)
        # x = Rearrange('b (n n n) c -> b n n n c')(x)
        ...
        self.blocks = nn.ModuleList(
            [
                SwinTransformerBlock(
                    ...
                    use_checkpoint=use_checkpoint,
                    attention=MixAttention if (i % 2 == 0) else WindowAttentionGlobal,
                )

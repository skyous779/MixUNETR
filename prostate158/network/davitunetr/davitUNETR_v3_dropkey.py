...
class davitUNETR(nn.Module):
    ...
    def forward(self, x_in):
        ...
        enc1 = self.encoder2(hidden_states_out[0])  # H / 2
        enc2 = self.encoder3(hidden_states_out[1])  # H / 4 
        enc3 = self.encoder4(hidden_states_out[2])  # H / 8
        dec4 = self.encoder10(hidden_states_out[4]) # H / 32
        dec3 = self.decoder5(dec4, hidden_states_out[3]) # H / 16
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        for i in range(depth):
            self.block_.append(SpatialBlock(
                    dim=dim,
                    num_heads=num_heads,
                    window_size=self.window_size,
        ...
        # for i in range(depth):
        #     self.block_.append(SwinTransformerBlock(
        #             dim=dim,
        #             num_heads=num_heads,
        #             window_size=self.window_size,
        #             shift_size=self.no_shift if (i % 2 == 0) else self.shift_size, 
...
if __name__=='__main__':
    ...
    label = torch.randn(2, 1, 96, 96, 96)
    # print(label.shape)
    print(input.shape)
